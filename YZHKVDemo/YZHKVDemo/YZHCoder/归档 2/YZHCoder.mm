...
{
    [YZHMachTimeUtils recordPointWithText:@"encodeInteger"];
    if (codeData == nil) {
        return;
    }
    val = integerToZigzag(val);
    [YZHMachTimeUtils recordPointWithText:@"interToZigzag"];
    uint8_t type = TYPE_LS(1, 7);
    ...
    type |= TYPE_AND(cnt - 1, BYTE_CNT_FIELD_MASK);
    type |= TYPE_LS(TYPE_AND(codeType, TYPE_FIELD_MASK), TYPE_FIELD_OFFSET);
    [YZHMachTimeUtils recordPointWithText:@"getType"];
    [codeData appendWriteByte:type];
    [YZHMachTimeUtils recordPointWithText:@"appendWrite"];

    [codeData ensureRemSize:cnt];
    [YZHMachTimeUtils recordPointWithText:@"ensureRemSize"];

    int64_t startOffset = [codeData dataSize];
    [YZHMachTimeUtils recordPointWithText:@"dataSize"];
    [self _encodeInt64:val toBuffer:codeData.bytes + startOffset];
    [YZHMachTimeUtils recordPointWithText:@"encodeInt64"];
    [codeData seekTo:startOffset + cnt];
    [YZHMachTimeUtils recordPointWithText:@"seekTo"];
}
